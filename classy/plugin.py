...
class ClassyPlugin(idaapi.plugin_t):
    ...
    def edit_pure_virtual_vals(self):
        ...
        txt = idaapi.ask_text(1024, ', '.join([('0x%X' % x) for x in db.pure_virtual_vals]),"Enter pure virtual values")
    ...
    def edit_deleted_virtual_vals(self):
        ...
        txt = idaapi.ask_text(1024, ', '.join([('0x%X' % x) for x in db.deleted_virtual_vals]), "Enter deleted virtual values")
        if txt is None or not txt.strip():
            return
